...
# ═══════════════════════════════════════════════════
# PENDING ACTIONS (for PDF uploads, etc.)
# ═══════════════════════════════════════════════════

pending_actions = {}  # {user_id: {"action": "pdf_summary", "data": ...}}


# ═══════════════════════════════════════════════════
# PDF COMMANDS (user sends /pdf then uploads file)
# ═══════════════════════════════════════════════════
...
async def pdf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_text(
            "📄 Send me a PDF file and I'll analyze it!\n\n"
            "Or use specific commands:\n"
            "/pdfask <question> — then send PDF\n"
            "/pdfflash — generate flashcards from PDF\n"
            "/pdfquiz — generate quiz from PDF\n"
            "/pdfkey — highlight key points"
        )
        pending_actions[user_id] = {"action": "pdf_summary"}
    except Exception as e:
        logger.error(f"PDF cmd error (uid={user_id}): {e}")


async def pdfask_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        question = " ".join(context.args) if context.args else ""
        if not question:
            await update.message.reply_text("❓ Usage: /pdfask <your question>\nThen send a PDF file.")
            return
        await update.message.reply_text("❓ Got it! Now send me the PDF file.")
        pending_actions[user_id] = {"action": "pdf_ask", "question": question}
    except Exception as e:
        logger.error(f"PDF ask error (uid={user_id}): {e}")


async def pdfflash_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        await update.message.reply_text("📚 Send me a PDF and I'll create flashcards from it!")
        pending_actions[user_id] = {"action": "pdf_flashcards"}
    except Exception as e:
        logger.error(f"PDF flash error (uid={user_id}): {e}")


async def pdfquiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        await update.message.reply_text("📝 Send me a PDF and I'll create a quiz from it!")
        pending_actions[user_id] = {"action": "pdf_quiz"}
    except Exception as e:
        logger.error(f"PDF quiz error (uid={user_id}): {e}")


async def pdfkey_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        await update.message.reply_text("🔑 Send me a PDF and I'll highlight key points!")
        pending_actions[user_id] = {"action": "pdf_key_points"}
    except Exception as e:
        logger.error(f"PDF key error (uid={user_id}): {e}")


async def notesflash_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        text = " ".join(context.args) if context.args else ""
        if not text:
            await update.message.reply_text("📚 Usage: /notesflash <your notes>\n\nExample: /notesflash Photosynthesis converts sunlight to energy")
            return
        await update.message.reply_text("⏳ Creating flashcards...")
        result = tools2.notes_to_flashcards(text)
        await safe_reply(update, f"📚 *Flashcards from Notes*\n\n{result}")
    except Exception as e:
        logger.error(f"Notes flash error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create flashcards.")
        except Exception:
            pass


async def notesquiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        text = " ".join(context.args) if context.args else ""
        if not text:
            await update.message.reply_text("📝 Usage: /notesquiz <your notes>")
            return
        await update.message.reply_text("⏳ Creating quiz...")
        result = tools2.notes_to_quiz(text)
        await safe_reply(update, f"📝 *Quiz from Notes*\n\n{result}")
    except Exception as e:
        logger.error(f"Notes quiz error (uid={user_id}): {e}")
        try:
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not document or not document.file_name or not document.file_name.lower().endswith(".pdf"):
            await update.message.reply_text("📄 Please send a PDF file.")
            return

        action_data = pending_actions.pop(user_id, None)
        action = action_data["action"] if action_data else "pdf_summary"

        wait_msg = await update.message.reply_text(f"📄 Processing PDF ({action})... ⏳")
        ...
        # Extract text
        pdf_text = tools2.extract_pdf_text(tmp_path)
        ...
        if pdf_text.startswith("__ERROR__"):
            await wait_msg.edit_text(f"❌ {pdf_text.replace('__ERROR__: ', '')}")
            return

        # Process based on action
        if action == "pdf_summary":
            result = tools2.pdf_summary(pdf_text)
        elif action == "pdf_ask":
            question = action_data.get("question", "Summarize this")
            result = tools2.pdf_ask(pdf_text, question)
        elif action == "pdf_flashcards":
            result = tools2.pdf_flashcards(pdf_text)
        elif action == "pdf_quiz":
            result = tools2.pdf_quiz(pdf_text)
        elif action == "pdf_key_points":
            result = tools2.pdf_key_points(pdf_text)
        else:
            result = tools2.pdf_summary(pdf_text)
        ...
        logger.error(f"Document handler error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to process PDF. Try again.")
        except Exception:
            pass
...
async def yt_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = tools2.yt_summary(url)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"YT error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Video analysis failed.")
        except Exception:
            pass


async def ytkey_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        url = " ".join(context.args) if context.args else ""
        if not url:
            await update.message.reply_text("🔑 Usage: /ytkey <YouTube URL>")
            return
        await update.message.reply_text("⏳ Extracting key points... ⏳")
        result = tools2.yt_key_points(url)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"YT key error (uid={user_id}): {e}")
        try:
...
async def ytnotes_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        url = " ".join(context.args) if context.args else ""
        if not url:
            await update.message.reply_text("📒 Usage: /ytnotes <YouTube URL>")
            return
        await update.message.reply_text("⏳ Generating notes... ⏳")
        result = tools2.yt_notes(url)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"YT notes error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to generate notes.")
        except Exception:
            pass


async def ytquiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        url = " ".join(context.args) if context.args else ""
        if not url:
            await update.message.reply_text("📝 Usage: /ytquiz <YouTube URL>")
            return
        await update.message.reply_text("⏳ Creating quiz... ⏳")
        result = tools2.yt_quiz(url)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"YT quiz error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create quiz.")
        except Exception:
            pass
...
async def savematerial_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        args = context.args if context.args else []
        if not args:
            await update.message.reply_text("💾 Usage: /savematerial <topic> | <content>\n\nExample: /savematerial Python | Lists are ordered collections")
            return
        full = " ".join(args)
        if "|" in full:
            topic, content = full.split("|", 1)
        else:
            topic = full[:50]
            content = full
        mat_id = db.save_material(user_id, topic.strip(), content.strip())
        await update.message.reply_text(f"💾 Material saved! (ID: {mat_id})\n\n*{topic.strip()}*")
    except Exception as e:
        logger.error(f"Save material error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to save material.")
        except Exception:
            pass


async def searchmaterial_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        keyword = " ".join(context.args) if context.args else ""
        materials = db.search_materials(user_id, keyword)
        if not materials:
            await update.message.reply_text("💾 No materials found." + (f" matching '{keyword}'" if keyword else ""))
            return
        text = "💾 *Your Study Materials*\n\n"
        for m in materials:
            text += f"📄 [{m['id']}] *{m['topic']}*\n{m['content'][:100]}{'...' if len(m['content']) > 100 else ''}\n\n"
        text += "/savematerial <topic> | <content> — Save\n/delmaterial <id> — Delete"
        await safe_reply(update, text)
    except Exception as e:
        logger.error(f"Search material error (uid={user_id}): {e}")
        try:
...
async def delmaterial_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        if not context.args:
            await update.message.reply_text("❌ Usage: /delmaterial <id>")
            return
        mat_id = int(context.args[0])
        if db.delete_material(user_id, mat_id):
            await update.message.reply_text(f"🗑️ Material #{mat_id} deleted.")
        else:
            await update.message.reply_text(f"❌ Material #{mat_id} not found.")
    except (ValueError, IndexError):
        await update.message.reply_text("❌ Usage: /delmaterial <id>")
    except Exception as e:
        logger.error(f"Del material error (uid={user_id}): {e}")


async def studyplan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        topics = " ".join(context.args) if context.args else ""
        if not topics:
            await update.message.reply_text("📚 Usage: /studyplan <topics>\n\nExample: /studyplan Python, Data Structures, Algorithms")
            return
        await update.message.reply_text("⏳ Creating study plan... ⏳")
        result = tools2.generate_study_plan(topics)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Study plan error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create study plan.")
        except Exception:
            pass


async def timetable_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        info = " ".join(context.args) if context.args else "student, 5 subjects, study 6am-10pm"
        await update.message.reply_text("⏳ Creating timetable... ⏳")
        result = tools2.generate_timetable(info)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Timetable error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create timetable.")
        except Exception:
            pass


async def currentaffairs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        await update.message.reply_text("📰 Fetching today's news... ⏳")
        result = tools2.current_affairs_summary()
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Current affairs error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to fetch current affairs.")
        except Exception:
            pass
...
async def remind_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not task:
            ...
            await safe_reply(update, text)
            return
        rem_id = db.add_reminder(user_id, task)
        await update.message.reply_text(f"⏰ Reminder set! (ID: {rem_id})\n\n*{task}*")
    except Exception as e:
        logger.error(f"Remind error (uid={user_id}): {e}")
        try:
...
async def delremind_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if db.delete_reminder(user_id, rem_id):
        ...
        logger.error(f"Del remind error (uid={user_id}): {e}")


# ═══════════════════════════════════════════════════
# ENHANCED WRITING COMMANDS
# ═══════════════════════════════════════════════════
...
async def essay_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        topic = " ".join(context.args) if context.args else ""
        if not topic:
            await update.message.reply_text("📝 Usage: /essay <topic>\n\nExample: /essay Impact of AI on Education")
            return
        await update.message.reply_text("⏳ Writing essay... ⏳")
        result = tools2.generate_essay(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Essay error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to generate essay.")
        except Exception:
            pass
...
async def grammar_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        text = " ".join(context.args) if context.args else ""
        if not text:
            await update.message.reply_text("✏️ Usage: /grammar <text>\n\nOr reply to a message with /grammar")
            return
        if update.message.reply_to_message and update.message.reply_to_message.text:
            text = update.message.reply_to_message.text
        await update.message.reply_text("⏳ Checking grammar... ⏳")
        result = tools2.grammar_correct(text)
        await safe_reply(update, f"✏️ *Grammar Check*\n\n{result}")
    except Exception as e:
        logger.error(f"Grammar error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to check grammar.")
        except Exception:
            pass
...
async def simplify_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        text = " ".join(context.args) if context.args else ""
        if not text:
            await update.message.reply_text("🧒 Usage: /simplify <text>\n\nOr reply to a message with /simplify")
            return
        if update.message.reply_to_message and update.message.reply_to_message.text:
            text = update.message.reply_to_message.text
        result = tools2.simplify_text(text)
        await safe_reply(update, f"🧒 *Simplified (ELI5)*\n\n{result}")
    except Exception as e:
        logger.error(f"Simplify error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to simplify.")
        except Exception:
            pass
...
async def expand_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        text = " ".join(context.args) if context.args else ""
        if not text:
            await update.message.reply_text("📝 Usage: /expand <short notes>")
            return
        await update.message.reply_text("⏳ Expanding... ⏳")
        result = tools2.expand_notes(text)
        await safe_reply(update, f"📝 *Expanded Answer*\n\n{result}")
    except Exception as e:
        logger.error(f"Expand error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to expand.")
        except Exception:
            pass
...
async def mindmap_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        topic = " ".join(context.args) if context.args else ""
        if not topic:
            await update.message.reply_text("🧠 Usage: /mindmap <topic>\n\nExample: /mindmap Machine Learning")
            return
        await update.message.reply_text("⏳ Creating mindmap... ⏳")
        result = tools2.generate_mindmap(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Mindmap error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create mindmap.")
        except Exception:
            pass


async def presentation_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        topic = " ".join(context.args) if context.args else ""
        if not topic:
            await update.message.reply_text("📊 Usage: /presentation <topic>\n\nExample: /presentation Climate Change")
            return
        await update.message.reply_text("⏳ Creating presentation... ⏳")
        result = tools2.create_presentation(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Presentation error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create presentation.")
        except Exception:
            pass
...
async def codeconvert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        args = context.args if context.args else []
        if len(args) < 2:
            await update.message.reply_text("🔄 Usage: /codeconvert <from_lang> <to_lang> <code>\n\nExample: /codeconvert python javascript print('hello')")
            return
        from_lang = args[0]
        to_lang = args[1]
        code = " ".join(args[2:])
        if not code:
            await update.message.reply_text("❌ Provide code to convert.")
            return
        await update.message.reply_text("⏳ Converting code... ⏳")
        result = tools2.convert_code(code, from_lang, to_lang)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Code convert error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Code conversion failed.")
        except Exception:
            pass


async def codequestions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        topic = " ".join(context.args) if context.args else ""
        if not topic:
            await update.message.reply_text("💻 Usage: /codequestions <topic>\n\nExample: /codequestions arrays")
            return
        await update.message.reply_text("⏳ Generating questions... ⏳")
        result = tools2.coding_questions(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Code questions error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to generate questions.")
        except Exception:
            pass


async def codequiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        topic = " ".join(context.args) if context.args else ""
        if not topic:
            await update.message.reply_text("📝 Usage: /codequiz <topic>\n\nExample: /codequiz Python basics")
            return
        await update.message.reply_text("⏳ Creating quiz... ⏳")
        result = tools2.coding_quiz(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Code quiz error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to create quiz.")
        except Exception:
            pass
...
async def codeimprove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        code = " ".join(context.args) if context.args else ""
        if not code:
            await update.message.reply_text("🔧 Usage: /codeimprove <code>")
            return
        await update.message.reply_text("⏳ Analyzing code... ⏳")
        result = tools2.code_improve(code)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Code improve error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Failed to improve code.")
        except Exception:
            pass


async def bugdetect_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        code = " ".join(context.args) if context.args else ""
        if not code:
            await update.message.reply_text("🐛 Usage: /bugdetect <code>")
            return
        await update.message.reply_text("⏳ Scanning for bugs... ⏳")
        result = tools2.detect_bugs(code)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Bug detect error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Bug detection failed.")
        except Exception:
            pass


# ═══════════════════════════════════════════════════
# RESEARCH & FACT-CHECKING COMMANDS
# ═══════════════════════════════════════════════════

async def factcheck_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        claim = " ".join(context.args) if context.args else ""
        if not claim:
            await update.message.reply_text("🔍 Usage: /factcheck <claim>\n\nExample: /factcheck The Great Wall of China is visible from space")
            return
        await update.message.reply_text("🔍 Fact-checking... ⏳")
        result = tools2.fact_check(claim)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Factcheck error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Fact-check failed.")
        except Exception:
            pass
...
async def research_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not topic:
            ...
            return
        await update.message.reply_text("🔬 Researching... This may take a moment ⏳")
        result = tools2.deep_research(topic)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Research error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Research failed.")
        except Exception:
            pass
...
async def verify_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        claim = " ".join(context.args) if context.args else ""
        if not claim:
            await update.message.reply_text("✅ Usage: /verify <claim or source>")
            return
        result = tools2.verify_source(claim)
        await safe_reply(update, f"✅ *Source Verification*\n\n{result}")
    except Exception as e:
        logger.error(f"Verify error (uid={user_id}): {e}")


async def compareanswers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = safe_get_user_id(update)
    try:
        question = " ".join(context.args) if context.args else ""
        if not question:
            await update.message.reply_text("⚖️ Usage: /compareanswers <question>\n\nExample: /compareanswers What is the best programming language?")
            return
        await update.message.reply_text("⏳ Comparing answers... ⏳")
        result = tools2.compare_answers(question)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Compare answers error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Comparison failed.")
        except Exception:
            pass
    ...
    try:
        ...
        if not goal:
            await update.message.reply_text(
                ...
                "I'll plan, research, and execute your goal autonomously!\n\n"
                "Usage: /agent <your goal>\n\n"
                "Example: /agent Create a marketing strategy for a coffee shop")
            return
        await update.message.reply_text("🤖 Agent activated! Working on your goal... ⏳")
        result = tools2.autonomous_agent(goal)
        await safe_reply(update, result)
    except Exception as e:
        logger.error(f"Agent error (uid={user_id}): {e}")
        try:
            await update.message.reply_text("❌ Agent task failed.")
        except Exception:
            pass
    ...
    try:
        ...
        if not url:
            await update.message.reply_text(
                "📥 *Video Download*\n\n"
                "Usage: /download <URL>\n\n"
                "Supports: YouTube, Instagram, Facebook, Twitter, and more.\n"
                "Max file size: 50MB\n\n"
                "Example: /download https://youtube.com/watch?v=xxx")
            return
        wait_msg = await update.message.reply_text("📥 Downloading video... This may take a moment ⏳")
        file_path = tools2.download_video(url)
        if file_path.startswith("__ERROR__"):
            await wait_msg.edit_text(f"❌ {file_path.replace('__ERROR__: ', '')}")
            return
        try:
            ...
            logger.error(f"Send video error: {send_err}")
            await wait_msg.edit_text(f"❌ Video downloaded but couldn't send (too large?). File saved at: `{file_path}`")
    except Exception as e:
        logger.error(f"Download error (uid={user_id}): {e}")
        try:
...
def setup_bot():
    ...
    # Document handler — catches PDF uploads
    application.add_handler(MessageHandler(
    ...
    # ═══════════════════════════════════════════════════
    # NEW FEATURE COMMANDS (Extended)
    # ═══════════════════════════════════════════════════

    # PDF Processing
    application.add_handler(CommandHandler("pdf", pdf_cmd))
    application.add_handler(CommandHandler("pdfask", pdfask_cmd))
    application.add_handler(CommandHandler("pdfflash", pdfflash_cmd))
    application.add_handler(CommandHandler("pdfquiz", pdfquiz_cmd))
    application.add_handler(CommandHandler("pdfkey", pdfkey_cmd))
    application.add_handler(CommandHandler("notesflash", notesflash_cmd))
    application.add_handler(CommandHandler("notesquiz", notesquiz_cmd))

    # YouTube
    application.add_handler(CommandHandler("yt", yt_cmd))
    application.add_handler(CommandHandler("ytkey", ytkey_cmd))
    application.add_handler(CommandHandler("ytnotes", ytnotes_cmd))
    application.add_handler(CommandHandler("ytquiz", ytquiz_cmd))

    # Study Tools
    application.add_handler(CommandHandler("savematerial", savematerial_cmd))
    application.add_handler(CommandHandler("searchmaterial", searchmaterial_cmd))
    application.add_handler(CommandHandler("delmaterial", delmaterial_cmd))
    application.add_handler(CommandHandler("studyplan", studyplan_cmd))
    application.add_handler(CommandHandler("timetable", timetable_cmd))
    application.add_handler(CommandHandler("currentaffairs", currentaffairs_cmd))
    application.add_handler(CommandHandler("remind", remind_cmd))
    application.add_handler(CommandHandler("delremind", delremind_cmd))

    # Enhanced Writing
    application.add_handler(CommandHandler("essay", essay_cmd))
    application.add_handler(CommandHandler("grammar", grammar_cmd))
    application.add_handler(CommandHandler("simplify", simplify_cmd))
    application.add_handler(CommandHandler("expand", expand_cmd))
    application.add_handler(CommandHandler("mindmap", mindmap_cmd))
    application.add_handler(CommandHandler("presentation", presentation_cmd))

    # Enhanced Coding
    application.add_handler(CommandHandler("codeconvert", codeconvert_cmd))
    application.add_handler(CommandHandler("codequestions", codequestions_cmd))
    application.add_handler(CommandHandler("codequiz", codequiz_cmd))
    application.add_handler(CommandHandler("codeimprove", codeimprove_cmd))
    application.add_handler(CommandHandler("bugdetect", bugdetect_cmd))

    # Research & Fact-checking
    application.add_handler(CommandHandler("factcheck", factcheck_cmd))
    application.add_handler(CommandHandler("research", research_cmd))
    application.add_handler(CommandHandler("verify", verify_cmd))
    application.add_handler(CommandHandler("compareanswers", compareanswers_cmd))
